...
//	private PorterDuffXfermode clearfXfermode = new PorterDuffXfermode(Mode.CLEAR);
	...
	public void setData(String ...args) {
		...
		this.sunrise = args[0];
		this.sunset = args[1];
		this.pressure = TextUtils.isEmpty(args[2])?"0":args[2];
        this.windspeed = TextUtils.isEmpty(args[3])?"0":args[3];
        this.winddirect = TextUtils.isEmpty(args[4])?"0":args[4];
		this.humidity = TextUtils.isEmpty(args[5])?"0":args[5];
		invalidate();
	}
...
//		setLayerType(View.LAYER_TYPE_SOFTWARE, null);
	...
	protected void onDraw(Canvas canvas) {
		...
		try {
			...
			canvas.drawText(getResources().getString(R.string.wind_speed)+"  "+windspeed+ "km/h " , fanHeight + textSize, -textSize, paint);
			canvas.drawText(getResources().getString(R.string.wind_direction)+"  " + winddirect, fanHeight + 2*textSize, textSize/2-4, paint);
			canvas.drawText(getResources().getString(R.string.wind_level)+"  " + humidity, fanHeight + 3*textSize, textSize+textSize-5, paint);
			...
			canvas.drawText(getResources().getString(R.string.pressure1)+" " + pressure + "hpa", pressureTextRight, sunArcHeight + paintTextOffset, paint);
			// draw astor info
		...
		try {
			...
			if (curTime >= srTime && curTime <= ssTime) {// 说明是在白天，需要画太阳
				...
				paint.setStyle(Style.FILL);
				paint.setColor(Color.parseColor("#FFDC1A"));
		...
		if (!visibleRect.isEmpty()) {
			//在下一个Frame开始的时候，发起一些invalidate操作
