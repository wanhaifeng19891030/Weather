...
public class AqiView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (aqi == 0) {
			...
			canvas.drawText(getResources().getString(R.string.no_data), w / 2f, h / 2f, textPaint);
			return;
		}
		float currAqiPercent = -1f;
		try {
			currAqiPercent = Float.valueOf(aqi) / 500f;// 污染%
		...
		if (currAqiPercent >= 0f) {
			...
			textPaint.setColor(0xffffffff);
			try {
				canvas.drawText(aqi + "", 0, lineSize * 3, textPaint);
			} catch (Exception e) {
			}
			textPaint.setTextSize(lineSize * 1f);
			textPaint.setColor(0x88ffffff);
			try {
				canvas.drawText(qlty + "", 0, lineSize * 4.25f, textPaint);
			} catch (Exception e) {
			}
	...
	public void setData(int aqi,String qlty) {
		if (aqi != 0) {
            this.aqi = aqi;
			this.qlty = qlty;
			invalidate();
